...
{ 
	...
	else
	{
		...
		depth = LineariserProfondeur(texture2D(depthMap, fragTexCoord.xy).x); 
		center_depth =  LineariserProfondeur(texture2D(depthMap, vec2(0.5,0.5)).x);
		...
		color = (1.0-w)*clear_color + blurred_col * w;
